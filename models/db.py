...
def STATIC(*args): # pylint: disable=invalid-name
	x = []
	if len(args) > 1:
		for arg in args[1:]:
			x.append(str(arg))

	return URL('static', str(args[0]), args=x)
...
# -------------------------------------------------------------------------
response.formstyle = config.get('forms.formstyle')  # or 'bootstrap3_stacked' or 'bootstrap2' or other
response.form_label_separator = config.get('forms.separator') or ''
...
mail.settings.server = 'logging' if request.is_local else config.get('smtp.server')
